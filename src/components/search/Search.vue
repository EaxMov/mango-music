<template>
  <div class="SearchBox">
    ...
    <div class="searchResult bystyle">
      ...
      <MvList class="layout" :showPublishTime="false" :mvlistArr="videoList" v-if="currentIndex === 3" />
  ...
  methods: {
    ...
    getSearchKeywords(type) {
      getSearchKeywords(this.searchkeywords, type).then((res) => {
        switch (type) {
          case 1:
            let trackIds = res.data.result.songs
              .map((item) => {
                return item.id;
              })
              .join(",");
          ...
          case 10:
            ...
            break;
          case 1014:
            this.videoList = res.data.result.videos.map(item => {
              return {
                imgurl16v9:item.coverUrl,
            ...
            break;
          case 1000:
            this.MusicMeuList = res.data.result.playlists;
            break;
        }
      });
    },
...
.meu li:before {
  ...
  height: 4px;
  opacity: 0;
  background-color: rgb(231, 190, 19, 0.5);
